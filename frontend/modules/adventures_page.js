...
async function fetchAdventures(city) {
  ...
  try {
  let adventur=await fetch(config.backendEndpoint + `/adventures?city=${city}`);
  let data=await adventur.json();
  return data;
  }
  catch(err) {
    return null;
...
function addAdventureToDOM(adventures) {
  ...
  adventures.forEach(element => {
    add(element.id,element.category,element.image,element.name,element.costPerHead,element.duration);
  });

  function add(id,category,image,name,costPerHead,duration){
    let domElement=document.getElementById("data");
    let temp= `
...
function filterByDuration(list, low, high) {
  ...
  list.filter(
    ...
    (i)=>{
      if(i.duration>=low && i.duration<=high){
        ans.push(i);
      }
    }
  );
